...
use tokenizer::TkTokenizer;

pub struct WorkingTokenizer {
    _arc: std::sync::Arc<()>,
    ptr: *const TkTokenizer,
}
impl WorkingTokenizer {
    ...
    pub unsafe fn new(tokenizer: &TkTokenizer, arc: std::sync::Arc<()>) -> Self {
        WorkingTokenizer {
            _arc: arc,
    ...
    fn perform(&self) -> Result<Self::Output, Self::Error> {
        match self {
            EncodeTask::Single(worker, input, add_special_tokens) => {
                ...
                let tokenizer: &TkTokenizer = unsafe { &*worker.ptr };
                tokenizer
                    .encode(
